...
class PDDL_Parser:
        ...
        if type(tokens) is list and tokens.pop(0) == 'define':
            ...
            while tokens:
                ...
                if   t == 'domain':
                    ...
                    self.parse_predicates(group, self.predicates)
                elif t == ':types':
                    self.parse_types(group, self.types)
                elif t == ':action':
    ...
    # Parse types
    #-----------------------------------------------
    def parse_types(self, group, types):
        ...
        while(group):
            ...
            if item != '\n':
                if(item == '-'):
                    name = group.pop(0)
                    if name != 'object':
                        extend = [x for x in types if x.name == name]
                else:
                    list_extend.append(item)
            else:
                for i in list_extend:
                    types.append(Type(i, extend))
                list_extend = []
        for t in types:
            print(t)
    ...
    # Parse predicates
    #-----------------------------------------------
    def parse_predicates(self, group, predicates): #TODO
        if not type(group) is list:
